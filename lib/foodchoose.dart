...
class _FoodChoosePageState extends State<FoodChoosePage> {
  ...
  void fetchParticipants() {
    _firestore
        ...
        .listen((snapshot) {
      if (snapshot.exists) {
        final participantDetails = List<Map<String, dynamic>>.from(
            snapshot.data()?['participantDetails'] ?? []);
        setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('투표 입력', style: TextStyle(color: Colors.white)),
                  ...
                  ElevatedButton(
                    ...
                    onPressed: () async {
                      ...
                      if (foodInput.isNotEmpty) {
                        final newFoods =
                            foodInput.split(',').map((e) => e.trim()).toSet();
                        ...
                        if (existingFoodsSnapshot.exists) {
                          final existingFoodsList = List<String>.from(
                              existingFoodsSnapshot['food'] ?? []);
                          existingFoods = existingFoodsList.toSet();
                        }

                        // 기존 데이터와 새로운 데이터 병합 및 중복 제거
                        final updatedFoods =
                            existingFoods.union(newFoods).toList();
